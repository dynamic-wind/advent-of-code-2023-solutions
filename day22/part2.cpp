...
struct Brick {
    ...
    void disintegrate(Grid& grid) const {
        const auto& [x1, y1, z1] = p1;
        const auto& [x2, y2, z2] = p2;
        for (int i = x1; i <= x2; i++) {
            for (int j = y1; j <= y2; j++) {
    ...
    void reintegrate(Grid& grid) const {
        const auto& [x1, y1, z1] = p1;
        const auto& [x2, y2, z2] = p2;
        for (int i = x1; i <= x2; i++) {
            for (int j = y1; j <= y2; j++) {
...
int main() {
    ...
    for (const Brick& brick : bricks) {
        brick.disintegrate(grid);
        int disintegrated_count = 0;
        vector<bool> been_disintegrated(bricks.size(), false);
        bool all_disintegrated = false;
        while (!all_disintegrated) {
            all_disintegrated = true;
            for (const Brick& other : bricks) {
                if (brick.id != other.id &&
                    !been_disintegrated.at(other.id-1) &&
                    other.can_be_lowered(grid))
                {
                    all_disintegrated = false;
                    other.disintegrate(grid);
                    been_disintegrated.at(other.id-1) = true;
                    disintegrated_count++;
                }
            }
        }
