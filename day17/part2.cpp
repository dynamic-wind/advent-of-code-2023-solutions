...
int main() {
    ...
    vector<vector<array<array<bool, 10>, 4>>> visited(lines.size(), vector(lines.at(0).size(), array<array<bool, 10>, 4>{}));
    {
        Node start;
        start.pos = ii(0, 0);
        start.total_cost = 0;
    ...
    while (pq.size() > 0) {
        ...
        for (int d = 0; d < int(offsets.size()); d++) {
            ...
            if ((n.pos != ii(0, 0) && dir != n.dir && n.dir_counter < 4) || (dir == n.dir && dir_counter > 10)) {
                continue;
            }
